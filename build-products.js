...
/* ==================================================================
   ...
   ================================================================== */
:root{
  --sand:#FAF6EF;--paper:#FFF;--ink:#2A2016;--mist:#6B6256;
  --clay:#A95A21;--clay-deep:#7E4114;--gold:#E9B978;
...
.btn.ghost{background:transparent;color:var(--txt);border:1px solid var(--edge,rgba(42,32,22,.58))}
...
.sz-cm{display:block;font-size:11.5px;color:var(--txt-soft);font-family:'Space Mono',monospace}
...
.sz-cap{caption-side:top;text-align:left;font-family:'Space Mono',monospace;font-size:11px;letter-spacing:1.5px;text-transform:uppercase;opacity:1;padding-bottom:8px}
...
.gal-hint{position:absolute;bottom:14px;right:14px;z-index:2;background:rgba(0,0,0,.78);color:#fff;backdrop-filter:blur(6px);font-family:'Space Mono',monospace;font-size:9.5px;letter-spacing:1.5px;text-transform:uppercase;padding:6px 11px;border-radius:999px;pointer-events:none}
...
.pdp-ans summary::after{content:'+';margin-left:auto;font-family:'Space Mono',monospace;color:var(--txt-soft)}
.pdp-ans[open] summary::after{content:'–'}
.pdp-ans-hint{font-family:'Space Mono',monospace;font-size:10.5px;letter-spacing:.3px;font-weight:400;color:var(--txt-soft)}
.shipcards{display:grid;gap:8px;margin:2px 0 10px}
.shipcard{display:grid;gap:4px;padding:12px 14px;border:1px solid var(--line);border-radius:3px;background:var(--card);font-size:13px;color:var(--txt-soft)}
.shipcard b{color:var(--txt);font-size:12.5px;letter-spacing:.3px}
.shipcard strong{color:var(--txt)}
...
.foot-copy{margin-top:14px;color:rgba(255,255,255,.85)}
...
/* ---- Mobile navigation ----------------------------------------------------
   Links were display:none below the breakpoint with nothing replacing them,
   so phones had no menu at all. Hamburger + slide-down panel. */
.mnav{display:none;position:relative;z-index:120;background:none;border:0;padding:8px;margin:-8px -6px -8px 0;cursor:pointer;line-height:0;color:inherit}
...
@media(max-width:820px){
  ...
  .m-panel{display:block;position:fixed;left:0;right:0;top:0;z-index:110;padding:78px 22px 26px;
    background:rgba(24,17,9,.97);-webkit-backdrop-filter:blur(14px);backdrop-filter:blur(14px);
  ...
  .m-panel a{display:block;padding:15px 4px;color:#F7EFE2;text-decoration:none;font-size:19px;
    letter-spacing:.3px;border-bottom:1px solid rgba(247,239,226,.12)}
  .m-panel a:last-child{border-bottom:0}
  .m-panel a.active,.m-panel a[aria-current="page"]{color:#E9B978}
...
.pdp-sizes-label{display:flex;align-items:center;justify-content:space-between;
  ...
  text-transform:uppercase;opacity:1;margin-bottom:9px}
.pdp-sizes{display:flex;gap:9px;flex-wrap:wrap;margin-bottom:16px;min-height:48px}
.pdp-loading{font-size:13px;color:var(--txt-soft);align-self:center}
...
<div class="note" data-sb-rotate><span class="sb-rot on"><span class="sb-ship-uae">↺ Free UAE delivery over AED 150 &nbsp;·&nbsp; ⚡ Same-day Dubai, order by 2pm &nbsp;·&nbsp; Next-day UAE-wide</span><span class="sb-ship-gcc">✈ GCC delivery 3&ndash;5 working days &nbsp;·&nbsp; Free over AED 400</span><span class="sb-ship-intl">✈ Worldwide delivery 7&ndash;14 working days</span></span><span class="sb-rot">✦ Founding Edition &nbsp;·&nbsp; a limited first run &nbsp;·&nbsp; ⚐ Designed in the UAE</span><span class="sb-rot">↺ 14-day returns &nbsp;·&nbsp; free within the UAE</span></div>
<nav>
...
${galShots(p).map((s,i)=>`
    ...
    <div class="buy">
      ...
      <!-- Pre-launch only. When the buy box is live it owns size selection, and a
           second, non-clickable row of the same sizes above it read as a broken
           control: a customer clicked it, nothing happened, and he assumed the
           store was not open. The scarcity line survives inside the buy box. -->
      ${LAUNCHED ? '' : `<div class="stock" id="stock" hidden>
      ...
      <!-- The answer cluster (Rastah benchmark, 29 Aug 2026): the questions a
           ...
           can I return it — answered UNDER the buy button, not in the footer.
      ...
      <div class="pdp-answers">
        <details class="pdp-ans">
          <summary>Delivery &amp; returns <span class="pdp-ans-hint"><span class="sb-ship-uae">free over AED 150 · same-day Dubai</span><span class="sb-ship-gcc">GCC 3–5 days · free over AED 400</span><span class="sb-ship-intl">worldwide 7–14 days</span></span></summary>
          <div class="shipcards">
            <!-- No regional-indicator (flag) emoji anywhere in these cards:
                 ...
                 the gibberish "SAQAOMBHKW" for every Windows visitor (caught
                 by Faheem on the preview). The UAE flag is a 4-rect inline
            ...
            <div class="shipcard shipcard-uae"><b><svg class="shipflag" viewBox="0 0 30 20" aria-hidden="true"><rect width="30" height="20" fill="#fff"/><rect width="30" height="6.7" fill="#00732F"/><rect y="13.3" width="30" height="6.7" fill="#000"/><rect width="8" height="20" fill="#FF0000"/></svg> United Arab Emirates</b>
              <span>Same-day Dubai — order by 2pm · AED 25</span>
              <span>Next-day, all seven emirates — AED 20, <strong>free over AED 150</strong></span></div>
        ...
        ${(p.faq && p.faq.length) ? `<details class="pdp-ans">
          ...
          ${p.faq.slice(0, 3).map(function (q) { return '<p class="pdp-ans-foot"><strong>' + esc(q.q) + '</strong><br>' + esc(q.a) + '</p>'; }).join('')}
          <p class="pdp-ans-foot"><a href="#faq">All questions &darr;</a></p>
        </details>` : ''}
      </div>
...
(function(){
  ...
  /* THE FIT BADGE MUST NEVER LIE (Faheem, 30 Aug). The model photos show the
     OVERSIZED cut; on a Regular (or polo) page the standing "REGULAR FIT"
     badge over an oversized-worn photo would visually promise the roomy drape
     the slim chart cannot deliver - the exact wrong-size path the runs-slim
...
(function(){
  ...
  var pdpStock=document.getElementById('pdpStock');
  /* Pre-launch the #stock block exists; live it does not, and the scarcity
     line goes in the buy box instead. Bail only if BOTH destinations are
     absent - the old guard returned on !box and would have killed the
     availability fetch the moment that block was removed. */
  if(!box && !pdpStock) return;
  ...
  fetch('https://'+SHOPIFY.domain+'/api/'+SHOPIFY.apiVersion+'/graphql.json',{
    ...
    if(row) row.innerHTML=sizes.map(function(sz){
    ...
    if(ps && msg){ ps.textContent=msg; ps.hidden=false; }
...
<script>
/* Mobile menu: links are cloned from the desktop nav so the two can't drift. */
(function(){
  ...
  panel.className='m-panel'; panel.id='mobileNav'; panel.setAttribute('aria-label','Menu');
  ...
  [].slice.call(src.querySelectorAll('a')).forEach(function(a){
    if(a.classList.contains('nav-cart')||a.classList.contains('cart-btn')) return;
  ...
  .then(function(d){
    ...
    variants.forEach(function(v){
      ...
      b.addEventListener('click',function(){
        ...
        var st=document.getElementById('pdpStock');
